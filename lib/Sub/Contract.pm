...
new contracts are automatically added to a contract pool.

The contract pool can be searched at runtime for contracts matching some conditions.
...
One may also find it handy to turn on or off memoizing for a
subroutine from the contract definition.

Those pragmatic considerations have shaped the API of Sub::Contract.
...
Constraints are specified inside the contract as code references to the test's
code.
...
will returns false or croaks. If it returns false, Sub::Contract will
emit an error looking as if the contractor croaked.

=head1 Contracts and context
...
How this affects a contractor contract is tricky and hard to define. For example,
if a function that always returns an array is called in scalar context, should
we consider it a contract breach?

For technical reasons, constraints defined with the C<out()> methods
...
no standard way of inheriting a parent class's contracts that has been
agreed upon. In Sub::Contract, child classes do not inherit contracts,
but any call to a contractor subroutine belonging to the parent class
from within the child class is verified against the contract (note that
this is true for all calls to any contractors).

=head2 Relevant error messages

To be usable, contracts must be specific about what failed. Therefore
...
=head1 Other contract APIs in Perl
...
original contract programming syntax defined in Eiffel. It also enables design-by-contract,
meaning that your classes are implemented inside the contract, rather than having class
implementation and contract definition as 2 different areas in a package declaration.
...
Empty the contractor's cache of result values.
