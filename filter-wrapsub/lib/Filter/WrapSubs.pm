...
#
#   $Id: WrapSubs.pm,v 1.4 2007-10-03 15:37:37 erwan_lemonnier Exp $
#
...
use accessors ('pkg',           # name of the package processed
	       ...
	       'subpdom',       # dom of the subroutine currently filtered
...
The problem with this solution is that *ALL* the subroutines
within the filtered package will be modified. Read on.
...
generate different inserted code for each subroutine. Even better,
we want to know about the subroutine currently filtered and about the
whole package.
...
The argument C<$filter> gives you access to the filtered
...
or within every return statement. This code can be specifically
generated for each subroutine.
...
names and the code is harder to follow in a debugger. And the results of the perl
profiler may become hard to interpret when filled with anonymous subs.
