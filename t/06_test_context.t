#-------------------------------------------------------------------
#
#   $Id: 06_test_context.t,v 1.5 2008-05-22 16:03:24 erwan_lemonnier Exp $
#
...
# no context with contract and memoization
{
    ...
    eval { foo(); };
    ok($@ =~ /calling memoized.cached subroutine main::foo in void context/, "die if memoized sub called in void context");

    # die even on second call with same (no) arguments
    eval { foo(); };
    ok($@ =~ /calling memoized.cached subroutine main::foo in void context/, "die even on second call");
}
